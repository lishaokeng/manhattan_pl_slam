...
7. 由于VINS前端的特征点使用光流进行前后帧跟踪，光流跟踪的召回率较低，图像质量不佳的情况下经常出现跟踪特征点较少的情况（低于100个），而最小二乘和极大似然方法又需要观测越多计算结果越准确，因此引入特征点深度滤波器。极大提高特征点跟踪数量和稳定性，进而保证最小二乘结果的准确性。（参考半直接法SVO和直接法DSO）；
