...
7. 由于VINS前端的特征点使用光流进行前后帧跟踪，光流跟踪优点是耗时低，缺点是：①在纹理较少或边缘上提取的fast点，跟踪精度极低，基本上都会跟踪错误（可视化显示可以很明显感受到）。②图像质量不佳时召回率较低（低于100个），而最小二乘和极大似然方法又需要观测越多计算结果越准确。这两点极大影响了位姿的计算精度，因此引入特征点深度滤波器。大大提高特征点跟踪数量和稳定性，进而保证最小二乘结果的准确性。这一步改进从根本上解决了光流跟踪的缺点，避免向后端图优化里引入错误数据（参考半直接法SVO和直接法DSO）；
