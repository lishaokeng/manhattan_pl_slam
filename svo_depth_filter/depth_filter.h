...
struct Seed
{
  ...
  vo::Feature* ftr;            //!< Feature in the keyframe for which the depth should be computed.
  ...
  Seed(vo::Feature* ftr, float depth_mean, float depth_min);
};
...
public:
  ...
  typedef std::function<void (vo::MapPoint*, double )> callback_t;
  ...
  DepthFilter(callback_t seed_converged_cb);
  ...
  void addFrame(vo::Frame *frame);

  /// Add new keyframe to the queue
  void addKeyframe(vo::Frame * frame, double depth_mean, double depth_min);
  ...
  void removeKeyframe(vo::Frame * frame);
  ...
  void getSeedsCopy(vo::Frame* frame, std::list<Seed>& seeds);
...
protected:
  ...
  std::thread* thread_;
  std::queue<vo::Frame*> frame_queue_;
  std::mutex frame_queue_mut_;
  std::condition_variable frame_queue_cond_;
  vo::Frame* new_keyframe_;               //!< Next keyframe to extract new seeds.
  ...
  void initializeSeeds(vo::Frame* frame);

  /// Update all seeds with a new measurement frame.
  virtual void updateSeeds(vo::Frame* frame);
