...
struct ELSEDParams {
  ELSEDParams(){}
  ELSEDParams(double validation){validationTh = validation;}
...
class ELSED {
 public:
  ...
  void InitializingAnchors(const cv::Mat &_image);

  Segments DetectAfterInitialized(const std::unordered_set<int> &matched_line_idx);
  ...
  void computeAnchorPoints(const cv::Mat &dirImage,
  ...
  ELSEDParams& GetParamsReference(){return params;}
