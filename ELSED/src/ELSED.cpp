...
void ELSED::InitializingAnchors(const cv::Mat &_image)
{
...
Segments ELSED::DetectAfterInitialized(const std::unordered_set<int> &matched_line_idx)
{
  std::vector<Pixel> new_anchors;
  new_anchors.reserve(anchors.size());
  for(auto &p : anchors)
  {
    int idx = p.y * imgInfo->imageWidth + p.x;
    if(matched_line_idx.find(idx) == matched_line_idx.end()) new_anchors.emplace_back(p);
  }
...
inline void ELSED::computeAnchorPoints(const cv::Mat &dirImage,
  ...
  full_anchors = cv::Mat(gradImage.size(), CV_8UC1, cv::Scalar(0));
  uchar *ptr_full_anchors = full_anchors.ptr<uchar>();

  int nAnchors = 0;
  int indexInArray;
  for (int w = 1, w_it = 0; w < imageWidth - 1; w++, w_it++)
  {
    bool flag1 = false;
    if(w_it >= scanInterval) w_it = 0;
    if(w_it == 0) flag1 = true;
    for (int h = 1, h_it = 0; h < imageHeight - 1; h++, h_it++)
    {
      bool flag2 = false;
      if(h_it >= scanInterval) h_it = 0;
      if(h_it == 0) flag2 = true;
      ...
      if (dirImg[indexInArray] == UPM_EDGE_HORIZONTAL)
      {
        ...
        if(gradImg[indexInArray] >= gradImg[indexInArray - imageWidth] &&
           gradImg[indexInArray] >= gradImg[indexInArray + imageWidth])
        {
            ptr_full_anchors[indexInArray] = 1;
            if (flag && gradImg[indexInArray] >= gradImg[indexInArray - imageWidth] + anchorThresh &&
      ...
      else
      {
        ...
        if(gradImg[indexInArray] >= gradImg[indexInArray - 1] &&
           gradImg[indexInArray] >= gradImg[indexInArray + 1])
        {
            ptr_full_anchors[indexInArray] = 1;
            if (flag && gradImg[indexInArray] >= gradImg[indexInArray - 1] + anchorThresh &&
